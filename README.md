...
- **VIEW mode — the camera-chain remote** — moves what the projectors are showing: the whole camera/surface chain of the output. Every output camera swings as **ONE motion around the center camera** (the surface whose slice sits in the middle of the projector canvas — it is the pivot, like standing in the middle of the room and turning your head):
  ...
  - **DOLLY slider** glides the chain toward/away along the center camera's view; **AUTO ORBIT** ping-pongs across the full 270° hands-free; **RESET** eases back to the saved poses.
  - The rig transform is applied at render time, so saved projects, sessions and undo history are never polluted; the studio's **CHAIN tab** drives the same rig from the desktop, and the `/output` info line shows the live sweep (`CHAIN AUTO -46°`).
