...
- **Live sync — in ANY browser, on ANY machine** — every edit made in the control (corner drags, mesh nodes, surface moves in the output editor, presets, blend, quality, canvas size, resolution) reaches `/output` within ~300 ms, through two transports that work together: **BroadcastChannel** (instant, same browser) and a **server relay** (`/api/projection/relay` — the studio POSTs its state, every output page follows over SSE). The relay is what makes the output a true live preview even when it runs in a **different browser or on the projector machine** — no shared storage, no portable link, no re-publish needed. The readout shows **LIVE LINK** while the studio is in contact and **HOLDING** when it has been closed — the picture keeps running its last state either way (a `?s=` link re-boots from its published snapshot), and `IMPORT .JSON` loads a project file anywhere.
...
### Smartphone remote control — QR code, camera gestures, button pad & camera-chain VIEW, real time

Press **REMOTE QR** in the projection studio topbar (or on the `/output` overlay): a modal shows a QR code plus the controller URL. Scan it with a phone and the phone becomes the controller — with **three modes** behind a segmented switch.

- **Auto-QR on the output screen** — the `/output` page shows a **small QR badge in the corner by itself** whenever no phone is connected (it sizes with the screen via `vmin`, so it stays subtle on any projector resolution). The moment a phone streams hands or pings its pad the badge fades out; when the phone leaves it comes back on its own. Tap it to open the full REMOTE QR modal.

- **CAMERA mode (`/remote` controller page)** — the phone's front camera is shown live (mirrored) with both hand skeletons drawn on top; **up to TWO hands** are tracked on the phone with the same local MediaPipe model, and the frames stream to the server at ~25 Hz. Open palm = attract, fist = caution, moving both hands like a swimming stroke = two flowing currents.
- **BUTTONS mode — the show pad** — a touch surface for one thumb:
  - **Action grid** — FEED (drop snacks), BURST (shockwave at the heart of the school), SHARK (predator pass), TURTLE / RAY (visitors), PULSE (light wave). Every press lands on the studio **and on every `/output` page at once** over `/api/remote/cmd` (SSE).
  - **Joystick** — steers the primary current: the stick streams a synthesized open-palm hand through the same `/api/remote/hands` pipeline, so the fish follow it on every page; flick fast and the gesture engine reads a swipe. Release and the water calms.
  - **SWIM / SOUND toggles** — badged with the studio's real state (the studio echoes `{ swim, muted }` back to the relay); **BOOST** is hold-to-swim-fast and only lights up while SWIM is on.
- **VIEW mode — the camera-chain remote** — moves what the projectors are showing: the whole camera/surface chain of the output. Every output camera swings as **ONE motion around the center camera** (the surface whose slice sits in the middle of the projector canvas — it is the pivot, like standing in the middle of the room and turning your head):
  - **Orbit pad** — drag left/right to sweep the chain (yaw), up/down to tilt it (pitch), streamed ~18 Hz.
  - **MOVE pad (XYZ)** — drag to translate the whole chain: left/right strafes along the center camera's right (X), up/down **raises/lowers the cameras (naik-turun, Y)** — one shared offset applied to every camera, so walls stay joined while the room appears to shift.
  - **270° linked sweep** — the yaw range spans ±135°; viewpoint chips (every 45°) and the YAW slider snap anywhere on it. Because the rig rotates every camera by the SAME offset around the pivot, all relative angles — and therefore every span-locked wall edge — stay connected while it moves: the picture glides without a single cut.
  - **DOLLY Z slider** glides the chain toward/away along the center camera's view; **LIFT Y slider** sets the height precisely; **AUTO ORBIT** ping-pongs across the full 270° hands-free (MOVE XYZ keeps working while it sweeps); **RESET** eases everything back to the saved poses.
  - The rig transform is applied at render time, so saved projects, sessions and undo history are never polluted; the studio's **CHAIN tab** drives the same rig from the desktop (YAW / PITCH / DOLLY Z / MOVE X / LIFT Y sliders), and the `/output` info line shows the live sweep (`CHAIN AUTO -46°`).
- **Every page follows, in real time** — the studio and every `/output` page (any browser, any machine on the network) subscribe over SSE. While the phone streams hands, they take priority over the local camera and drive the same two-point force field the local hands use — the HUD shows **REMOTE HANDS** and the QR modal turns green (**PHONE CONNECTED** — it stays green in buttons/view mode too, via the pad heartbeat). Stop the phone and every page falls back to local tracking automatically.
...
- **HTTPS note** — phones only grant camera access on secure origins. `http://localhost` works; over the LAN run `npm run dev:https` (self-signed cert) or open the app through its HTTPS link. The QR modal and the controller page show an explicit warning when the origin is insecure — the **BUTTONS and VIEW pads work on plain http too**.
