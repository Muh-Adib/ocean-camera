...
### Smartphone remote control — QR code, camera gestures & button pad, real time

Press **REMOTE QR** in the projection studio topbar (or on the `/output` overlay): a modal shows a QR code plus the controller URL. Scan it with a phone and the phone becomes the controller — with **two modes** behind a segmented switch.
...
- **HTTPS note** — phones only grant camera access on secure origins. `http://localhost` works; over the LAN run `npm run dev:https` (self-signed cert) or open the app through its HTTPS link. The QR modal and the controller page show an explicit warning when the origin is insecure — the **BUTTONS pad works on plain http too**.
